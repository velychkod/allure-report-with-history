...
(async () => {
    try {
        // Determine platform-specific Allure binary
        const allureBinary = os.platform() === 'win32'
            ? path.join(__dirname, 'allure.bat')
            : path.join(__dirname, 'allure');

        if (os.platform() !== 'win32') {
            fs.chmodSync(allureBinary, 0o755); // ensure executable permission on Linux/macOS
        }

        console.log('Generating Allure report...');
        execSync(`"${allureBinary}" generate "${allureResultsPath}" --clean -o "./allure-report"`, { stdio: 'inherit' });

        // Prepare temp folder
        fs.removeSync(tempDir);
        fs.mkdirSync(tempDir);

        // Copy report to temp folder
        fs.copySync('./allure-report', tempDir);

        // Clone published branch if keeping reports
        if (reportsToKeep > 0) {
        ...
        // Archive current report with timestamp if keeping reports
        if (reportsToKeep > 0) {
        ...
        // Generate index.html with links to all reports if requested
